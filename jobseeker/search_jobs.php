    ...
    <div class="row g-4">
        ...
        if ($jobs && mysqli_num_rows($jobs) > 0):
            while ($job = mysqli_fetch_assoc($jobs)): 
                $saved = (int) mysqli_fetch_column(mysqli_query($conn, "SELECT COUNT(*) FROM saved_jobs WHERE user_id = $uid AND job_id = " . (int)$job['id']));
        ?>
                ...
                <div class="card card-job h-100 <?php echo !empty($job['featured']) ? 'card-featured' : ''; ?>">
                    <div class="card-body">
                        ...
                        <div class="d-flex gap-2">
                            <a href="<?php echo BASE_URL; ?>job_detail.php?id=<?php echo (int)$job['id']; ?>" class="btn btn-sm btn-primary">View</a>
                            <?php if ($saved): ?>
                                <a href="saved_jobs.php?unsave=<?php echo (int)$job['id']; ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-bookmark-fill"></i></a>
                            <?php else: ?>
                                <a href="save_job.php?id=<?php echo (int)$job['id']; ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-bookmark"></i></a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            <div class="col-12">
                <div class="card p-5 text-center">
                    ...
                    <p class="mb-0 text-muted">No jobs match your criteria.</p>
                </div>
            </div>
        <?php endif; ?>
    </div>
